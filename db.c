...
/* default värde om användare saknas */
...
time( &today ) ;
strftime( title.created, 26,"%Y-%m-%d %H:%M:%S", localtime(&today) ) ;

  /* om nivå 0 på objekt så spara fšrst i filen */


if (( title.indent == 0 ) || ( title.object > 2 ))
  {
    fprintf( group, "%d\t%d\t%d\t%d\t%d\t", title.id, title.subject, 
        title.security, title.owner, title.indent ) ;
    fprintf( group, "%s\t%d\t%d\t%s\t%s\t%s\t%s\n", title.created, title.object,
         title.group, title.name, title.email, title.author, title.company ) ;
   ...
   while ( fgets ( string, 512, tmp ) != NULL )
     {
      ...
      fputs ( string, group  ) ;
       ...
       if ( title.subclass == id )    /* kommentar till detta inlägg */
        {
          fprintf( group, "%d\t%d\t%d\t%d\t%d\t", title.id, title.subject, 
                        title.security, title.owner, title.indent) ;
          fprintf( group, "%s\t%d\t%d\t%s\t%s\t%s\t%s\n", title.created, title.object,
...
while ( fgets ( string, 512, tmp ) != NULL )
{
 ...
 sprintf( string, "%d\t%d\t%d\t%d\t%d\t%s\t%d\t%d\t%s\t%s\t%s\t%s\n", title.id, title.subject, 
        title.security, title.owner, title.indent, title.created, title.object,
         title.group, title.name, title.email, title.author, title.company ) ;
 fputs( string, group ) ;
...
deletePublication ( struct _publication *publication )
...
if ((lock = lockSubject ( publication->category, publication->subject )) == NULL ) 
  return ( FALSE ) ;

if ( strcmp( EDITION, "Limited"))
  sprintf( filename, "%s/news-%d-%d.dat", DB_PATH, publication->category, publication->subject ) ;
else
  sprintf( filename, "%s/news-%d.dat", DB_PATH, publication->subject ) ;

sprintf( tmpname, "%s/news-%d.tmp", DB_PATH, publication->subject ) ;
...
title.security = article->security ;
title.group = article->group ;
title.indent = article->indent ;     
title.object = 2 ;                     /* inlägg */
title.subclass = article->comment ;   /* föregående inlägg */

strcpy ( title.created, article->created ) ;
strcpy ( title.name, article->title ) ;
strcpy ( title.email, article->email ) ;
...
if ((lock = lockSubject ( article->category, article->subject )) == NULL ) 
 { 
  errorHTML ("Fel vid oppning av lasfil") ;
  return ( FALSE ) ;
  }  
    /* spara ned id, rubrik, författare och created i fil */        
...
insertPublication ( struct _publication *publication )
...
publication->id = GetNextTitleId ( publication->category, publication->subject ) ;

if ( publication->id == ERROR ) 
  return ( FALSE ) ;
    
    /* spara ned id, rubrik, författare och created i fil */        

title.id = publication->id ;
title.category = publication->category ;
title.subject = publication->subject ;
title.owner = publication->owner ;
title.security = publication->security ;
title.indent = publication->media ;     
title.object = 9 ;                      /* publikation */
title.subclass = publication->comment ;   /* föregående inlägg */
title.group = publication->group ; 

strcpy ( title.created, publication->created ) ;
strcpy ( title.name, publication->title ) ;
strcpy ( title.email, publication->email ) ;
strcpy ( title.author, publication->author ) ;
strcpy ( title.company, publication->company ) ;
...
if ( strcmp( EDITION, "Limited"))
  sprintf( filename, "%s/message-%d-%d-%d.dat",DB_PATH,  publication->category, publication->subject, publication->id  ) ;
else 
  sprintf( filename, "%s/message-%d-%d.dat", DB_PATH, publication->subject, publication->id ) ; 
...
updatePublication ( struct _publication *publication )
...
updateTitle ( title ) ;
    ...
    /* öppna filen för skrivning av en publication */
...
title.security = message->security ;
title.indent = message->indent ; ;
title.object = 3 ;                              /* meddelande */

strcpy ( title.name, message->title ) ;
strcpy ( title.email, message->email ) ;
strcpy ( title.author, message->author ) ;
strcpy ( title.company, message->company ) ;
strcpy ( title.created, message->created ) ;
...
insertMessage ( struct _message *message )
...
title.id = message->id ;
title.subclass = message->comment ;
title.category = message->category ;
title.subject = message->subject ;
title.group = message->group ;
title.owner = message->owner ;
...
strcpy ( title.name, link->title ) ;
strcpy ( title.href, link->href ) ;
strcpy ( title.email, link->email ) ;
...
strcpy ( title.name, link->title ) ;
strcpy ( title.href, link->href ) ;
strcpy ( title.created, link->created ) ;
strcpy ( title.email, link->email ) ;
strcpy ( title.author, link->author ) ;
strcpy ( title.company, link->company ) ;
...
fprintf( fp, "%d\t%d\t%d\t%d\t%s\t%s\t%s\n", 
         category->id, category->owner, category->security, 0,
         category->created, 
...
fprintf( fp, "%d\t%d\t%d\t%d\t%s\t%s\t%s\n", 
         subject->id, subject->owner, subject->security, 
         subject->article, subject->created, 
  ...
  for (j=0;i<(int)strlen(string);i++,j++ ) 
   {
    if ( string[i] == '\t' )
      break ;      
    subject->created[j] = string[i] ;
   }
  subject->created[j] = (char) NULL ;
  i++ ;      
  ...
  for (j=0;i<(int)strlen(string);i++,j++ ) 
   {
    if ( string[i] == '\t' )
      break ;      
    category->created[j] = string[i] ;
   }
  category->created[j] = (char) NULL ;
  i++ ;      
...
char temp[20] ;         /* omvandling string -> heltal */
  ...
  i++ ;
   ...
   for (j=0;i<(int)strlen(string);i++,j++)
    ...
    if ( string[i] == '\t' )
      break ;      
    cur_title->created[j] = string[i] ;
    }
   cur_title->created[j] = (char) NULL ;
   i++ ;        
  ...
  for (j=0;i<(int)strlen(string);i++,j++)
   ...
   for (j=0;i<(int)strlen(string);i++,j++)
    ...
    if ( string[i] == '\t' )
      break ;      
    cur_title->name[j] = string[i] ;
    }
   cur_title->name[j] = (char) NULL ;
   i++ ;
   ...
   for (j=0;i<(int)strlen(string);i++,j++)
    ...
    if ( string[i] == '\t' )
      break ;      
    cur_title->email[j] = string[i] ;
    }
   cur_title->email[j] = (char) NULL ;
   i++ ;        
...
article->security = title.security ;
article->group = title.group ;

strcpy ( article->title, title.name ) ;
strcpy ( article->author, title.author ) ;
strcpy ( article->email, title.email ) ;
strcpy ( article->created, title.created ) ;
...
selectPublication ( int category_id, int subject_id, int object_id, struct _publication *publication )
...
publication->category = title.category ;
publication->subject = title.subject ;
publication->id = title.id ;
publication->owner = title.owner ;
publication->security = title.security ;
publication->comment = title.subclass ;
publication->group = title.group ;
publication->media = title.indent ;

strcpy ( publication->title, title.name ) ;
strcpy ( publication->author, title.author ) ;
strcpy ( publication->email, title.email ) ;
strcpy ( publication->created, title.created ) ;
strcpy ( publication->company, title.company ) ;
...
while ( fgets ( string, 255, fp ) != NULL )
  strcat( publication->text, string ) ;
...
message->security = title.security ;

strcpy ( message->title, title.name ) ;
strcpy ( message->author, title.author ) ;
strcpy ( message->email, title.email ) ;
strcpy ( message->created, title.created ) ;
     ...
     {             
       if ( strlen(article->email) > 0 )
         sprintf ( command, "%s -r%s -s\"%s\" %s < %s/message-%d-%d-%d.dat", 
                  MAIL, article->email, article->title, to, DB_PATH, 
